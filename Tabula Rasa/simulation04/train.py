...
# ── QPSK constellation (extracted from Sionna once, then pure PyTorch) ───────
_constel = sn.mapping.Constellation("qam", num_bits_per_symbol=2)
_mapper  = sn.mapping.Mapper(constellation=_constel)
with torch.no_grad():
    _bits = torch.tensor([[0,0],[0,1],[1,0],[1,1]], dtype=torch.float32)
    _syms = _mapper(_bits).squeeze(-1)                      # [4] complex
del _constel, _mapper

# ── Run bookkeeping ───────────────────────────────────────────────────────────
RUNS_DIR = os.path.join(os.path.dirname(__file__), "..", "artifacts", "sim04")
...
CONST_POINTS = _syms.to(device)   # [4] complex
CONST_BITS   = _bits.to(device)   # [4, 2]

# ── Pure-PyTorch QPSK helpers (no Sionna in the hot loop) ────────────────────
def generate_qpsk(batch_size, n_symbols):
    """Generate random QPSK symbols and bits directly on device."""
    idx = torch.randint(0, 4, (batch_size, n_symbols), device=device)
    return CONST_POINTS[idx], CONST_BITS[idx]  # [B,N] complex, [B,N,2]


def qpsk_demapper(rx_flat, no=1.0):
    """APP demapper for QPSK. rx_flat: [N] complex → [N,2] LLRs."""
    d2 = (rx_flat.unsqueeze(-1) - CONST_POINTS).abs().pow(2)   # [N, 4]
    log_p = -d2 / no
    llrs = []
    for k in range(2):
        mask0 = (CONST_BITS[:, k] == 0)
        mask1 = (CONST_BITS[:, k] == 1)
        # LLR = log P(bit=1) - log P(bit=0), matching Sionna's convention
        llrs.append(torch.logsumexp(log_p[:, mask1], dim=1)
                    - torch.logsumexp(log_p[:, mask0], dim=1))
...
def save_plot(steps_done):
    ...
    axes[0].set_title(f"2-agent cooperative jammer (NSF direct-gradient) — Kurtosis Detector\n"
...
for step in range(TOTAL_STEPS):
    ...
    tx_syms, tx_bits = generate_qpsk(BATCH_SIZE, N_SYMBOLS)
    ...
    # ── Soft BER loss ────────────────────────────────────────────────────────
    llr          = qpsk_demapper(rx_syms.reshape(-1), no=1.0).clamp(-10, 10)
    wrong_labels = 1.0 - tx_bits.reshape(-1, 2)
    ...
    del kurt, kurt_i, kurt_q, llr, wrong_labels
