...
from jammer_env import JammerEnv, N_SYMBOLS, DETECTION_THRESHOLD, BETA

# ── Run config (change these between runs) ────────────────────────────────────
TOTAL_STEPS = 1_000_000

# ── Auto-increment plot filename ──────────────────────────────────────────────
...
SNAPSHOT_STEPS = 500   # save this many actions per snapshot window
...
class LoggingCallback(BaseCallback):
    def __init__(self):
        ...
        self.detections  = []
        self.powers      = []
        self.actions     = []   # all raw actions, for IQ snapshots
...
# ── Plots ─────────────────────────────────────────────────────────────────────
W = max(1, TOTAL_STEPS // 100)   # window scales with run length

def rolling(data):
    return np.convolve(data, np.ones(W) / W, mode='valid')

meta = (f"run={run_id}  |  steps={TOTAL_STEPS:,}  |  N_SYMBOLS={N_SYMBOLS}  |  "
        f"BETA={BETA}  |  DET_THRESH={DETECTION_THRESHOLD}  |  rolling_W={W}")

fig, axes = plt.subplots(3, 1, figsize=(10, 9), sharex=True)

axes[0].plot(rolling(callback.bers))
axes[0].set_ylabel("BER (rolling avg)")
axes[0].set_title(f"PPO Jammer — Lossless Channel\n{meta}", fontsize=9)
...
axes[2].plot(rolling(callback.powers), color='green')
axes[2].axhline(DETECTION_THRESHOLD, color='orange', ls='--', label=f'Threshold ({DETECTION_THRESHOLD})')
axes[2].set_ylabel("Jammer Rx power (rolling avg)")
...
for ax, (label, chunk) in zip(axes, thirds):
    iq = chunk.reshape(-1, N_SYMBOLS, 2)   # (steps, N_SYMBOLS, 2)
    I  = iq[:, :, 0].flatten()
    Q  = iq[:, :, 1].flatten()
